...
import { StyleSheet, View, ScrollView, ActivityIndicator } from 'react-native' // todo sort these in alph
import {Header, Box} from './src/components'

const currencyApiUrl = 'https://api.coinmarketcap.com/v1/ticker/?limit=50'
...
export default class App extends React.Component {
  ...
  constructor (props) {
    ...
    var comp = this
    var updateData = function (currencyApiUrl) {
    ...
    updateData(currencyApiUrl)
    setInterval(function () { updateData(currencyApiUrl) }, 30000)
  };
      ...
      <View style={{ flex: 1 }}>
        ...
        <ScrollView style={[styles.Body, {flex: 4}]}>
          ...
          {this.state.currencies.map(function draw (currency) {
            return <Box percent_change_1h={currency.percent_change_1h} symbol={currency.symbol} price_usd={currency.price_usd} key={currency.name} name={String(currency.name)} />
          })
              }
        </ScrollView>
      </View>
